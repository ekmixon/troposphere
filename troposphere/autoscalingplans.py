...
class TagFilter(AWSProperty):
    props = {"Values": ([str], False), "Key": (str, True)}


class ApplicationSource(AWSProperty):
    props = {"CloudFormationStackARN": (str, False), "TagFilters": ([TagFilter], False)}


class PredefinedScalingMetricSpecification(AWSProperty):
    props = {"ResourceLabel": (str, False), "PredefinedScalingMetricType": (str, True)}


class MetricDimension(AWSProperty):
    props = {"Value": (str, True), "Name": (str, True)}
