...
class DeploymentCircuitBreaker(AWSProperty):
    ...
    props = {"Enable": (boolean, True), "Rollback": (boolean, True)}
...
class RepositoryCredentials(AWSProperty):
    props = {"CredentialsParameter": (str, False)}
...
class ContainerDependency(AWSProperty):
    props = {"Condition": (str, True), "ContainerName": (str, True)}
...
class AuthorizationConfig(AWSProperty):
    props = {"AccessPointId": (str, False), "IAM": (str, False)}
