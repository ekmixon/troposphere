...
def validate_growth_type(growth_type):
    if growth_type not in VALID_GROWTH_TYPE:
        raise ValueError(
            f'DeploymentStrategy GrowthType must be one of: {", ".join(VALID_GROWTH_TYPE)}'
        )
...
def validate_replicate_to(replicate_to):
    if replicate_to not in VALID_REPLICATION_DESTINATION:
        raise ValueError(
            f'DeploymentStrategy ReplicateTo must be one of: {", ".join(VALID_REPLICATION_DESTINATION)}'
        )
...
def validate_validator_type(validator_type):
    if validator_type not in VALID_VALIDATOR_TYPE:
        raise ValueError(
            f'ConfigurationProfile Validator Type must be one of: {", ".join(VALID_VALIDATOR_TYPE)}'
        )
